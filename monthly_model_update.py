...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/monthly_update.log'),
...
class MonthlyModelUpdater:
    ...
    def __init__(self, 
        ...
        logger.info(f"月次更新システム初期化完了: {self.update_date.strftime('%Y年%m月')}")
    ...
    def verify_environment(self) -> bool:
        ...
        if file_size < 50:  # 50MB未満
            ...
            logger.info(f"✅ データファイルサイズ: {file_size:.1f}MB")
        ...
        if free_space < 10:  # 10GB未満
            ...
            logger.info(f"✅ ディスク容量: {free_space:.1f}GB")
    ...
    def backup_current_model(self) -> bool:
        ...
        if not self.backup_old_model:
            logger.info("📦 バックアップはスキップされました")
            return True
        
        logger.info("📦 現在のモデルをバックアップ中...")
        ...
        try:
            ...
            if os.path.exists(self.current_model_path):
                ...
                logger.info(f"✅ モデルバックアップ: {backup_model_path}")
            ...
            if os.path.exists(self.current_config_path):
                ...
                logger.info(f"✅ 設定バックアップ: {backup_config_path}")
            ...
            if os.path.exists("logs/model_performance.json"):
                ...
                logger.info(f"✅ 性能記録バックアップ: {backup_perf_path}")
    ...
    def train_new_model(self) -> bool:
        """新しいモデルを学習"""
        logger.info(f"🧠 新モデル学習開始...")
        logger.info(f"   データ: {self.new_data_path}")
        logger.info(f"   サンプル: {self.sample_size:,}")
        logger.info(f"   モデル数: {self.n_models}")
        logger.info(f"   エポック: {self.epochs}")
    ...
    def evaluate_new_model(self) -> bool:
        ...
        try:
            ...
            logger.info(f"📊 新モデル性能:")
            logger.info(f"   ECE改善前: {self.new_performance['ece_original']:.4f}")
            logger.info(f"   ECE改善後: {self.new_performance['ece_improved']:.4f}")
            logger.info(f"   改善幅: {self.new_performance['ece_improvement']:+.4f}")
            logger.info(f"   最適温度: {self.new_performance['optimal_temperature']:.3f}")
    ...
    def compare_performance(self) -> bool:
        ...
        if new_improvement > 0.02:
            logger.info(f"   ✅ 大幅改善達成: {new_improvement:+.4f}")
            quality_checks.append(True)
        elif new_improvement > 0.01:
            logger.info(f"   ✅ 改善達成: {new_improvement:+.4f}")
            quality_checks.append(True)
        else:
        ...
        if self.previous_performance:
            ...
            logger.info("📈 前回モデルとの比較:")
            
            if new_ece < prev_ece:
                logger.info(f"   ✅ ECE改善: {prev_ece:.4f} → {new_ece:.4f}")
                quality_checks.append(True)
            else:
            ...
            if new_improvement > prev_improvement:
                logger.info(f"   ✅ 改善幅向上: {prev_improvement:+.4f} → {new_improvement:+.4f}")
                quality_checks.append(True)
            else:
                logger.info(f"   📊 改善幅比較: {prev_improvement:+.4f} → {new_improvement:+.4f}")
        
        # 総合判定
        success_ratio = sum(quality_checks) / len(quality_checks)
        
        if success_ratio >= 0.8:
            logger.info(f"🎉 新モデル採用推奨: 品質チェック {sum(quality_checks)}/{len(quality_checks)} 通過")
            return True
    ...
    def deploy_new_model(self) -> bool:
        ...
        try:
            ...
            logger.info(f"✅ 設定ファイル更新: {self.current_config_path}")
    ...
    def _generate_mt5_config(self):
        ...
        try:
            ...
            logger.info(f"✅ MT5設定ファイル生成: {mt5_config_path}")
    ...
    def run_monthly_update(self) -> bool:
        """月次更新の実行"""
        logger.info("🚀" * 30)
        logger.info(f"    月次モデル更新開始: {self.update_date.strftime('%Y年%m月')}")
        logger.info("🚀" * 30)
        ...
        try:
            ...
            # レポート表示
            print("\n" + report)
            logger.info(f"📄 更新レポート保存: {report_file}")
